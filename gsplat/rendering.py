...
def rasterization_imp(
    ...
    if colors.shape[-1] > channel_chunk:
        # slice into chunks
        n_chunks = (colors.shape[-1] + channel_chunk - 1) // channel_chunk
        render_colors, render_alphas, render_imps = [], [], []
        for i in range(n_chunks):
            colors_chunk = colors[..., i * channel_chunk : (i + 1) * channel_chunk]
            backgrounds_chunk = (
                backgrounds[..., i * channel_chunk : (i + 1) * channel_chunk]
            ...
            render_colors_, render_alphas_, render_imps_ = count_imp_wrapper(
                means2d,
                conics,
                colors_chunk,
                opacities,
            ...
            render_colors.append(render_colors_)
            render_alphas.append(render_alphas_)
            render_imps.append(render_imps_)
        render_colors = torch.cat(render_colors, dim=-1)
        render_alphas = render_alphas[0]  # discard the rest
        render_imps = torch.cat(render_imps, dim=-1)
    else:
        render_colors, render_alphas, render_imps = count_imp_wrapper(
            means2d,
            conics,
            colors,
            opacities,
    ...
    return render_colors, render_alphas, render_imps, meta
